...
describes what users get. Pick a format with `-o`, or let duckduckgo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
duckduckgo <command> -o table     # a rounded, color-aware grid for reading
duckduckgo <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
duckduckgo <command> -o list      # one record per section, easy on the eyes
duckduckgo <command> -o jsonl     # one JSON object per line, for piping
duckduckgo <command> -o json      # a single JSON array
duckduckgo <command> -o csv       # spreadsheet friendly
duckduckgo <command> -o tsv       # tab-separated
duckduckgo <command> -o url       # just the URL column
duckduckgo <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
duckduckgo <command> --fields id,title,url
```
...
```bash
duckduckgo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
duckduckgo <command>            # a table, because this is a terminal
duckduckgo <command> | wc -l    # JSONL, because this is a pipe
```
